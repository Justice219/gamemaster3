...
local function openMenu()
    ...
    frame:SetTitle("Gamemaster 3 v1.32")
    ...
    frame.Navbar:AddButton("Dash", lyx.Colors.Primary, lyx.Icons.Info, function(pnl)
        pnl:Clear()
        ...
        dashTable:SetSize(pnl:GetWide() - lyx.Scaling.ScaleW(30), lyx.Scaling.ScaleH(100))
        dashTable:SetPos(lyx.Scaling.ScaleW(15), lyx.Scaling.ScaleH(25))

        local richInfo = vgui.Create("RichText", pnl)
        richInfo:SetSize(pnl:GetWide() - lyx.Scaling.ScaleW(30), lyx.Scaling.ScaleH(100))
        richInfo:SetPos(lyx.Scaling.ScaleW(15), lyx.Scaling.ScaleH(150))
    ...
    frame.Navbar:AddButton("Tools", lyx.Colors.Primary, lyx.Icons.Stats, function(pnl)
        pnl:Clear()
        ...
        toolInfo:SetSize(390, lyx.Scaling.ScaleH(230))
        toolInfo:SetPos(lyx.Scaling.ScaleW(550), lyx.Scaling.ScaleH(25))
        toolInfo:TDLib()
        toolInfo:ClearPaint()   
            :Background(lyx.Colors.Primary, 6)
            :Outline(lyx.Colors.Primary, 1)
        local toolTitle = toolInfo:Add("DLabel")
            toolTitle:SetText("Tool Information")
            toolTitle:SetFont("lyx.font.title")
            toolTitle:SetTextColor(lyx.Colors.White)
            toolTitle:Dock(TOP)
            toolTitle:SizeToContents()
            toolTitle:DockMargin(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10), 0, 0)
        local toolName = toolInfo:Add("DLabel")
            toolName:SetText("Tool Name: ")
            toolName:SetFont("lyx.font.subtitle")
            toolName:SetTextColor(lyx.Colors.White)
            toolName:Dock(TOP)
            toolName:SizeToContents()
            toolName:DockMargin(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10), 0, 0)
        local toolDesc = toolInfo:Add("RichText")
            toolDesc:SetText("Tool Description: ")
            toolDesc:Dock(TOP)
            toolDesc:SizeToContents()
            toolDesc:DockMargin(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10), 0, 0)
        ...
        local function addArguments()
            ...
            toolArguments:SetSize(lyx.Scaling.ScaleW(150), lyx.Scaling.ScaleH(100))
            toolArguments:SetPos(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(75))
            toolArguments:Dock(TOP)
            toolArguments:DockMargin(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(20), lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10))
        end

        local toolButton = toolInfo:Add("lyx_button")
        toolButton:SetText("Run")
        toolButton:SetSize(lyx.Scaling.ScaleW(150), lyx.Scaling.ScaleH(15))
        toolButton:SetPos(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(190))
        toolButton:Inverse()
        toolButton:Dock(BOTTOM)
        toolButton:DockMargin(lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10), lyx.Scaling.ScaleW(10), lyx.Scaling.ScaleH(10))
        --! END OF RUNNING + ARGUMENTS !--

        --? LIST OF TOOLS !--
        -- search bar
        local search = vgui.Create("lyx_entry", pnl)
        search:SetSize(pnl:GetWide() - lyx.Scaling.ScaleW(750), lyx.Scaling.ScaleH(20))
        search:SetPos(lyx.Scaling.ScaleW(650), lyx.Scaling.ScaleH(5))
        ...
        local function addTools()
            ...
            toolsTable:SetSize(pnl:GetWide() - lyx.Scaling.ScaleW(750), lyx.Scaling.ScaleH(230))
            toolsTable:SetPos(lyx.Scaling.ScaleW(15), lyx.Scaling.ScaleH(25))
        end
        addTools()
                ...
                if (string.find(k, self:GetValue())) then
                    toolsTable:AddButton(k, {
                        ...
                        callback = function()
                            ...
                            chat.AddText(Color(0,238,255), "You have selected the tool: ", lyx.Colors.White, k)
                        end
                    }) 
                end
            end
    ...
    frame.Navbar:AddButton("Ranks", lyx.Colors.Primary, lyx.Icons.Sell, function(pnl)
        pnl:Clear()
        local r = "admin"
        ...
        button.DoClick = function()
            if r then
                net.Start("gm3:rank:add")
                    net.WriteString(r)
                net.SendToServer()
                chat.AddText(Color(0,255,242), "[GM3] ", lyx.Colors.White, "Added rank " .. r)
                frame:Remove()
            end
        end
        ...
        for k,v in pairs(gm3.ranks) do
            rankList:AddButton(k, {
                ...
                callback = function(box, value)
                    r = k
                    chat.AddText(Color(0,255,242), "[GM3] ", lyx.Colors.White, "Selected rank " .. k .. " for deletion.")
                end
            })
        end
        rankList:Setup()
