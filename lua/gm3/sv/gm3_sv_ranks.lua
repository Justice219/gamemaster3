...
function gm3:RankAdd(name)
    if gm3.ranks[name] then
        gm3:Log("Couldnt add Rank, it already exists: " .. name)    
    return end
    
    gm3.ranks[name] = true
    lyx:JSONSave("gm3_ranks.txt", gm3.ranks)
    gm3:Log("Added Rank: " .. name)
end

function gm3:RankRemove(name)
    if not gm3.ranks[name] then
        gm3:Log("Couldnt remove Rank, it doesnt exist: " .. name)    
    return end
    
    gm3.ranks[name] = nil
    lyx:JSONSave("gm3_ranks.txt", gm3.ranks)
    gm3:Log("Removed Rank: " .. name)
end
...
timer.Simple(1, function()
    ...
    if ranks then
        gm3.ranks = ranks
        gm3:Log("Loading ranks...")
        for k, v in pairs(gm3.ranks) do
            gm3:Log("Loaded Rank: " .. k)
        end
    else 
        gm3:Log("No ranks found, creating default ranks")
