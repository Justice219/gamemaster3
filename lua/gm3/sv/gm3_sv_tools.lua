...
function gm3:addTool(tool)
    if gm3.tools[tool.name] then
        gm3:Log("Tool with name " .. tool.name .. " already exists!")
    end

    gm3.tools[tool.name] = tool
    gm3:Log("Added tool with name " .. tool.name .. " to the tool list!")
end
...
function gm3:getTool(name)
    if gm3.tools[name] then
        ...
        gm3:Log("Tool with name " .. name .. " does not exist!")
    end
end
...
function gm3:runTool(name, ply, args)
    ...
    gm3:Log("Ran tool with name " .. name .. "!" .. "ply: " .. ply:SteamID())
end
